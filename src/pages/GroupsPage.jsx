...
import {
  createGroup, getGroups, sendInvite,
    ...
    <main className="pt-24 pb-16 px-6 bg-cream min-h-screen">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <h1 className="font-display font-bold text-3xl sm:text-4xl text-moss mt-3">Eco Groups</h1>
          <p className="font-body text-bark/55 mt-2 text-sm">Team up with friends and compete on the leaderboard.</p>
        </div>
                    ...
                    <div key={invite.$id} className="flex items-center justify-between bg-eco-50 rounded-2xl p-4 gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="font-body text-xs text-bark/60 mt-0.5">
                          ...
                          {invite.inviterEmail
                            ? <span className="text-bark/40"> · {invite.inviterEmail}</span>
                            : null}
                        </p>
                      </div>
              ...
              <>
                ...
                {showCreate && (
                  <div className="bg-white rounded-3xl border border-eco-100 p-7">
                    ...
                    <p className="font-body text-bark/55 text-sm mb-5">Start a new group and invite friends to join.</p>
                  ...
                  <div className="bg-white rounded-3xl border border-eco-100 p-7">
                    <div className="flex items-start justify-between mb-6">
                      ...
                      <div className="text-right">
                        <div className="font-display font-bold text-2xl text-eco-600">{activeGroup.totalPoints}</div>
            ...
            {loading ? (
              ...
              <div className="space-y-2">
                {leaderboard.map((g, index) => {
                  const isMyGroup = (profile?.groupIds || []).includes(g.$id);
                  return (
                    <div
                      key={g.$id}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className={`font-mono text-xs font-bold w-5 ${isMyGroup ? 'text-eco-300' : 'text-bark/40'}`}>
                          #{index + 1}
                        </span>
                        <span className={`font-body font-medium text-sm ${isMyGroup ? 'text-cream' : 'text-bark'}`}>
                          {g.name}
                        </span>
                      </div>
                      <span className={`font-mono text-xs font-bold ${isMyGroup ? 'text-eco-300' : 'text-eco-600'}`}>
                        {g.totalPoints} pts
                      </span>
                    </div>
                  );
